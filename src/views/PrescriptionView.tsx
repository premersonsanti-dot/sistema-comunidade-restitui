...
import { PrescriptionItem, Patient, Prescription, Medication, Profile, Clinic } from '../types';
...
interface PrescriptionViewProps {
  ...
  onAddPatient: (patient: Omit<Patient, 'id' | 'clinic_id'>) => Promise<Patient | undefined>;
  onSavePrescription: (prescription: Omit<Prescription, 'id' | 'clinic_id'>) => void | Promise<void>;
...
export const PrescriptionView: React.FC<PrescriptionViewProps> = ({
  ...
  const [doctorName, setDoctorName] = useState(() => localStorage.getItem('medsys_doctor_name') || userProfile?.full_name || '');
  const [doctorCrm, setDoctorCrm] = useState(() => localStorage.getItem('medsys_doctor_crm') || (userProfile ? `${userProfile.council_type} ${userProfile.council_number}` : ''));
