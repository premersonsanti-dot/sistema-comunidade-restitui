...
export const LoginView: React.FC<LoginViewProps> = ({ onLogin, onRegister, onGoogleLogin, onForgotPassword }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isRegistering) {
      ...
      onRegister({ name, email: user, password: pass, clinicCode: institutionCode });
    } else {
      ...
      <div className="w-full max-w-[380px] z-10 transition-all">
        ...
        <div className="bg-white rounded-[1.5rem] p-5 sm:p-6 shadow-2xl transition-all duration-300">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            {isRegistering && (
              <div className="space-y-1.5">
                <label className="text-slate-700 text-[10px] font-bold uppercase tracking-wider ml-1">Código da Instituição (Opcional)</label>
                <input
                  className="w-full h-12 bg-slate-50 border-slate-200 rounded-xl px-4 text-sm focus:ring-2 focus:ring-primary outline-none transition-all uppercase"
