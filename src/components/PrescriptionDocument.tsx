...
import { PrescriptionItem, Patient, Profile, Clinic } from '../types';
...
export const PrescriptionDocument: React.FC<PrescriptionDocumentProps> = ({
    ...
    const finalDoctorName = doctorName || doctorProfile?.full_name || '';
    const finalDoctorCrm = doctorCrm || `${doctorProfile?.council_type || ''} ${doctorProfile?.council_number || ''}`.trim();
        ...
        <div className={`w-full bg-white aspect-[1/1.414] p-8 flex flex-col text-slate-800 relative overflow-hidden ${isPrintCopy ? 'print:shadow-none print:w-full print:h-full print:m-0 print:border-none' : 'max-w-[500px] paper-shadow rounded-sm p-12 print:m-0 print:p-12 print:shadow-none print:w-full'}`}>
            ...
            <div className="flex justify-start mb-8">
                {clinicSettings?.logo_url ? (
                    <img src={clinicSettings.logo_url} alt="Logo Clínica" className="h-20 object-contain" />
                ) : (
                    <img src="/logo.jpg" alt="Logo Clínica" className="h-20 object-contain opacity-20 grayscale" />
                )}
            </div>
            ...
            <div className="mt-auto flex flex-col items-center text-center">
                ...
                <div className="w-full border-t border-gold/30 mt-6 pt-4 text-[9px] text-gray-500 flex flex-col items-center leading-tight">
                    <p>Telefones: {clinicSettings?.phones || ''}</p>
                    <p>{clinicSettings?.address || ''}</p>
                    <p>CNPJ: {clinicSettings?.cnpj || ''} - E-mail: {clinicSettings?.email || ''}</p>
                </div>
            </div>
